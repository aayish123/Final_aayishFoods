  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            {dbCategories.length > 0 ? (
              dbCategories.map((category) => (
                <Link key={category.id} to={`/menu?category=${category.name}`} className="flex flex-col items-center group">
                  <div className="w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 rounded-full border-4 border-[#cda274] overflow-hidden mb-4 shadow-xl group-hover:scale-105 transition-transform duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {cmsContent.trust_badges.list?.map((badge: TrustBadge, i: number) => (
              <div key={badge.id || i} className="flex items-center justify-center space-x-4 border-b sm:border-b-0 border-accent/10 pb-4 sm:pb-0 last:border-b-0 last:pb-0 sm:border-r sm:even:border-r-0 lg:even:border-r lg:last:border-r-0">
      ...
      <section className="bg-[#FDF8F0] flex flex-col md:flex-row border-b border-border/20 overflow-hidden">
        ...
        <div className="w-full md:w-1/2 relative min-h-[300px] md:min-h-[500px] border-b-4 md:border-b-0 md:border-r-4 border-accent/20 flex flex-col items-center justify-center text-center p-8 bg-accent/5">
